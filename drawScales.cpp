...
void MainWindow::drawAzimuthScales(float azCur360)
{
    ...
    float drctrAz180 = convert360angle2PlusMinus180(console2User.srvDrvInfo.directrisaAzimuth);
    float crntAz180 = convert360angle2PlusMinus180(console2User.srvDrvInfo.currentAzimuth);
...
{
    //qDebug()<<"drawScale360 directrisaAzimuth:"<<shMemC2U.srvDrvInfo.directrisaAzimuth<<"azCur:"<<azCur<<"currentAzimuth:"<<shMemC2U.srvDrvInfo.currentAzimuth<<"azCurScan:"<<azCurScan;
...
{
    float azCurScan = console2User.srvDrvInfo.currentAzimuth;//azCur;
    if(azCurScan > 180)
        azCurScan -= 360;
